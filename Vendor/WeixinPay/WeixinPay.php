...
class Weixinpay {
    ...
    public function unifiedOrder($order){
        ...
        $config=array(
            ...
            'spbill_create_ip'=>'120.76.220.167',
     ...
     */
    public function _getParameters($redirect_uri){
        // 如果没有get参数没有code；则重定向去获取openid；
        if (!isset($_GET['code'])) {
            // 获取订单号
            $out_trade_no=I('get.out_trade_no',1);
            // 返回的url
//            $redirect_uri=U('V1/Pay/pay','','',true);
            $redirect_uri=urlencode($redirect_uri);
            $url='https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$this->config['APPID'].'&redirect_uri='.$redirect_uri.'&response_type=code&scope=snsapi_base&state='.$out_trade_no.'#wechat_redirect';
            redirect($url);
        }else{
            // 如果有code参数；则表示获取到openid
            $code=I('get.code');
            // 取出订单号
            $out_trade_no=I('get.state',0);
            // 组合获取prepay_id的url
            ...
            $where['is_del'] = 0;
            $order_model = M('Order');
            $find = $order_model->field('body,total_fee,circle_id,goods_id,trading')->where($where)->find();
            $body = $find['body'];
            $total_fee = $find['total_fee'];
            $product_id = 0;
            if($find['trading']==1){
                $product_id = $find['circle_id'];
            }else if($find['trading']==2){
                $product_id = $find['goods_id'];
            }
            $order=array(
    ...
    public function getParameters($out_trade_no='',$openid=''){
        ...
        $where['is_del'] = 0;
        $order_model = M('Order');
        $find = $order_model->field('body,total_fee,product_id,trading')->where($where)->order('created_time desc')->find();
        $body = $find['body'];
        $total_fee = $find['total_fee'];
